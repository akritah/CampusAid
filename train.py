...
try:
    from backend.app.ml.classifier import TextClassifier
    from backend.app.ml.embedder import get_embedding
except ImportError:
    sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "backend"))
    TextClassifier = importlib.import_module("app.ml.classifier").TextClassifier
    get_embedding = importlib.import_module("app.ml.embedder").get_embedding
...
DEFAULT_LABELS_CSV = None  # Can be None if complaints.csv has category column
DEFAULT_MODEL_DIR = "./backend/models"


def load_training_data(complaints_csv: str, labels_csv: str = None) -> tuple:
    """
    Load complaint text and labels from CSV files.
    
    Args:
        complaints_csv: Path to complaints CSV file
        labels_csv: Path to labels CSV file (optional if complaints.csv has category column)
    ...
    try:
        ...
        if 'text' in complaints_df.columns:
            ...
            texts = complaints_df.iloc[:, 1].tolist()
        
        # Extract labels - try from complaints file first
        labels = None
        if 'category' in complaints_df.columns:
            labels = complaints_df['category'].tolist()
            logger.info(f"Using category column from {complaints_csv}")
        elif 'label' in complaints_df.columns:
            labels = complaints_df['label'].tolist()
            logger.info(f"Using label column from {complaints_csv}")
        
        # If no labels in complaints file, load from separate file
        if labels is None and labels_csv:
        ...
        if labels is None:
            raise ValueError("No category/label column found. Please provide labels.")
...
Examples:
  python train.py
  python train.py --complaints-csv ./data/complaints.csv --labels-csv ./data/labels.csv
  python train.py --model-dir ./backend/models --test-size 0.3
        """
    )
